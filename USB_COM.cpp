...
N_Communication::USB_COM_UTC::USB_COM_UTC(int in_length_8b, int out_length_8b ,N_Communication::Vendor_id_Hex _vendor_id, N_Communication::Product_id_Hex _product_id,
                         ...
                         frame_id(0), endpoint_in(_endpoint_in), endpoint_out(_endpoit_out), if_print(false){
    usb_in_data = new usb_utc_rx_cmd_t();
    usb_data_drv = new usb_utc_rx_data_t();
}

N_Communication::USB_COM_UTC::~USB_COM_UTC() {
    delete usb_in_data;
    delete usb_data_drv;
}

void N_Communication::USB_COM_UTC::USB_Com_Start_Trans_Asy(void (*cbf_wrapper)(struct libusb_transfer *)) {
    libusb_fill_interrupt_transfer(transfer_rx, get_device_handle(), endpoint_in, rx_buff, usb_message_in_length
    , cbf_wrapper,this,0);
...
void N_Communication::USB_COM_UTC::Deal_In_Data() {
...
void N_Communication::USB_COM_UTC::USB_In_CBF(struct libusb_transfer *transfer) {
...
void N_Communication::USB_COM_UTC::Print_Rx_Data() {
    std::cerr << "[Frame " << frame_id << " ] [Device " << get_product_id() << " UTC DATA] " << usb_data_drv->real_utc  << " [Date] " << usb_data_drv->date_utc <<
